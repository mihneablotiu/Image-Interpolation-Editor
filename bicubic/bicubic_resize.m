function R = bicubic_resize(I, p, q)
    ...
    for y = 0 : p - 1
        for x = 0 : q - 1
            ...
            coord_p(1) = coord_p(1) - x1;
            coord_p(2) = coord_p(2) - y1;
            
            % calculeaza valoarea interpolata a pixelului (x, y)
            % Obs: pentru scrierea in imagine, x si y sunt in coordonate de
            % la 0 la n - 1 si trebuie aduse in coordonate de la 1 la n
            R(y + 1, x + 1) = [1, coord_p(1), coord_p(1) * coord_p(1), coord_p(1) * coord_p(1) * coord_p(1)] * A * [1, coord_p(2), coord_p(2) * coord_p(2), coord_p(2) * coord_p(2) * coord_p(2)]';
        endfor
    endfor
